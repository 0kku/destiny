...
import type { TReactiveProperties } from "./TReactiveProperties.ts";
...
>(
  ...
  if (![null, Object].includes(input.constructor as ObjectConstructor)) {
    throw new TypeError(
      `Illegal object ${
        describeType(input)
      } passed to \`reactiveProperties()\`. You must either pass in objects that have \`Object\` or \`null\` as their prototype, or wrap the objects using \`ReactiveValue\`.`,
    );
  }
