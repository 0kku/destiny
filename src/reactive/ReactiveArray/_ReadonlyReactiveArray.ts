import { ReactiveArray } from "./_ReactiveArray.ts";
import {
  stronglyHeldDependencies,
  weaklyHeldDependencies,
} from "./arrayDependencyCaches.ts";
import { makeNonPrimitiveItemsReactive } from "./makeNonPrimitiveItemsReactive.ts";
import { internalArrays } from "./internalArrays.ts";
import { splicers } from "./splicers.ts";
...
import { internalSetReactiveValue } from "../ReactiveValue/internalSetReactiveValue.ts";
...
export class ReadonlyReactiveArray<InputType> {
  ...
  readonly #callbacks: Set<TReactiveArrayCallback<TArrayValueType<InputType>>> =
    new Set();

  // deno-lint-ignore ban-types People can pass literally anything into ReactiveArray
  readonly #consumers = new IterableWeakMap<
    object,
    TReactiveArrayCallback<TArrayValueType<InputType>>
  >();
  ...
  *[Symbol.iterator](): Iterable<TArrayValueType<InputType>> {
  ...
  async *[Symbol.asyncIterator](): AsyncIterable<
    TArrayUpdateArguments<TArrayValueType<InputType>>
  > {
    while (true) {
  ...
  #nextUpdate(): Promise<TArrayUpdateArguments<TArrayValueType<InputType>>> {
    return new Promise<[number, number, ...Array<TArrayValueType<InputType>>]>(
      (resolve) => {
        const cb: TReactiveArrayCallback<TArrayValueType<InputType>> = (
          ...props
        ) => {
          resolve(props);
  ...
  get(
    ...
    const i = (
      index < 0 ? this.#value.length + index : index
    );
  ...
  >(
    ...
    const reactor = new ReadonlyReactiveValue(
      callback(...this.#argsForFullUpdate()),
    );
    const fn = () =>
      internalSetReactiveValue.get(reactor)(
        callback(...this.#argsForFullUpdate()),
      );
  ...
  #splice(
    ...
    if (start > this.#value.length) {
      throw new RangeError(
        `Out of bounds assignment: tried to assign to index ${start}, but array length was only ${this.#value.length}. Sparse arrays are not allowed. Consider using .push() instead.`,
      );
    }

    this.#adjustIndices(start, deleteCount, items);
    const reactiveItems = makeNonPrimitiveItemsReactive(items, this);
    const deletedItems = this.#value.splice(
      start,
      deleteCount,
      ...reactiveItems,
    );
  ...
  filter(
    ...
    const filteredArray: ReactiveArray<TArrayValueType<InputType>> =
      new ReactiveArray();
    ...
    this.bind(
      (start, deletes, ...items) => {
        ...
        const lastInMask = maskArray.slice(0, start).reverse().find((v) =>
          v.show
        );
        ...
        const deletedItemCount =
          deletedMaskEntries.filter((v) => v.show).length;
   ...
   */
  flat(): ReadonlyReactiveArray<
    TUnwrapReactiveArray<TArrayValueType<InputType>>
  > {
    ...
    const cb = (
      ...
      callback(
        ...
        this.#indices[i]?.readonly ??
          throwExpression("Internal error: lost track of index"),
        this,
      );
  ...
  reduce(
    ...
    ReturnType<Array<TArrayValueType<InputType>>["reduce"]>
  > {
  ...
  reduceRight(
    ...
    ReturnType<Array<TArrayValueType<InputType>>["reduceRight"]>
  > {
  ...
  findIndex(
    ...
    ReturnType<Array<TArrayValueType<InputType>>["findIndex"]>
  > {
  ...
  >(
    ...
    const newArray = this.clone() as ReactiveArray<
      TArrayValueType<InputType> | U
    >;
    this.bind(
    ...
    function currentOffset(
      ...
      for (let i = 0; i < cutoff; i++) {
        tally += lengthTally[i]?.value ??
          throwExpression("Internal error: failed to concatenate", RangeError);
      }
      return tally;
    }
    for (const [i, item] of items.entries()) {
